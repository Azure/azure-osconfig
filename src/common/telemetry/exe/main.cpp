...
void shutdown(OsConfigLogHandle log, int exit_code) {
    CloseLog(&log);
    return exit_code;
}
...
int main(int argc, char* argv[]) {
    ...
    if (arg_index < argc) {
        try {
            ...
            OsConfigLogError(g_log, "Error: Invalid teardown time argument '" + std::string(argv[arg_index]) + "'. Must be a valid integer.");
            shutdown(g_log, 1);
        }
    }
    ...
    if (!file_exists(filepath)) {
        OsConfigLogError(g_log, "Error: File '" + filepath + "' does not exist.");
        shutdown(g_log, 1);
    }
    ...
    OsConfigLogInfo(g_log, init_message);
    ...
    if (init_success) {
        ...
        OsConfigLogInfo(g_log, "Processed telemetry JSON file: " + filepath);

        auto start_time = std::chrono::high_resolution_clock::now();
        telemetry.Shutdown();
        auto end_time = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
        OsConfigLogInfo(g_log, "Telemetry shutdown successfully! [" + std::to_string(duration.count()) + " ms]");

        // Delete the JSON file
        if (std::remove(filepath.c_str()) != 0) {
            OsConfigLogError(g_log, "Warning: Failed to delete JSON file: " + filepath);
        }
    } else {
