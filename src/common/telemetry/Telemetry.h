...
#define OSConfigTelemetryStatusTrace(callingFunctionName, status) OSConfigTelemetryStatusTraceImpl((callingFunctionName), (status), __LINE__)
...
#define OSConfigTelemetryStatusTraceImpl(callingFunctionName, status, line) {\
        ...
        telemetry_json = FormatAllocateString(\
            ...
            "\"Version\":\"%s\""\
            "}",\
            _timestamp ? _timestamp : TELEMETRY_NOTFOUND_STRING, __FILE__, line, __func__,\
            _ruleCodename ? _ruleCodename : TELEMETRY_NOTFOUND_STRING,\
            (callingFunctionName) ? (callingFunctionName) : TELEMETRY_NOTFOUND_STRING, status,\
            _resultString,\
            _scenarioName,\
            _elapsed_us,\
            _distroName ? _distroName : TELEMETRY_NOTFOUND_STRING,\
            _correlationId ? _correlationId : TELEMETRY_NOTFOUND_STRING,\
            OSCONFIG_VERSION);\
...
#define OSConfigTelemetryBaselineRun(baselineName, mode, durationSeconds) {\
        ...
        telemetry_json = FormatAllocateString(\
            ...
            "\"Version\":\"%s\""\
            "}",\
            timestamp ? timestamp : TELEMETRY_NOTFOUND_STRING,\
            (baselineName) ? (baselineName) : TELEMETRY_NOTFOUND_STRING,\
            (mode) ? (mode) : TELEMETRY_NOTFOUND_STRING,\
            durationSeconds, distroName ? distroName : TELEMETRY_NOTFOUND_STRING,\
...
#define OSConfigTelemetryRuleComplete(componentName, objectName, objectResult, microseconds) {\
        ...
        telemetry_json = FormatAllocateString(\
            ...
            "}",\
            timestamp ? timestamp : TELEMETRY_NOTFOUND_STRING,\
            (componentName) ? (componentName) : TELEMETRY_NOTFOUND_STRING,\
            (objectName) ? (objectName) : TELEMETRY_NOTFOUND_STRING,\
            objectResult,\
            microseconds, distroName ? distroName : TELEMETRY_NOTFOUND_STRING,\
            correlationId ? correlationId : TELEMETRY_NOTFOUND_STRING,\
            OSCONFIG_VERSION);\
        if (NULL != telemetry_json) {\
            TelemetryAppendPayloadToFile(telemetry_json);\
            FREE_MEMORY(telemetry_json);\
...
#define OSConfigTimeStampSave() do {} while (0)
#define OSConfigGetElapsedTime(elapsed_us_var) do {(elapsed_us_var) = 0;} while (0)
#define OSConfigGetElapsedTime(elapsed_us_var) do {(elapsed_us_var) = 0;} while (0)
#define OSConfigTelemetryStatusTrace(callingFunctionName, status) do {(void)(callingFunctionName); (void)(status);} while (0)
#define OSConfigTelemetryStatusTraceImpl(callingFunctionName, status, line) do {(void)(callingFunctionName); (void)(status); (void)(line); } while (0)
#define OSConfigTelemetryBaselineRun(baselineName, mode, durationSeconds) do {(void)(baselineName); (void)(mode); (void)(durationSeconds);} while (0)
#define OSConfigTelemetryRuleComplete(componentName, objectName, objectResult, microseconds) do {(void)(componentName); (void)(objectName); (void)(objectResult); (void)(microseconds); } while (0)
