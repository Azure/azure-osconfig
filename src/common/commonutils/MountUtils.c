                ...
                ((NULL != mountType) && (NULL != mountStruct->mnt_type) && (NULL != strstr(mountStruct->mnt_type, mountType))))
            {
                ...
                if (NULL != hasmntopt(mountStruct, desiredOption))
                {
                    OsConfigLogInfo(log, "CheckFileSystemMountingOption: option '%s' for mount directory '%s' or mount type '%s' found in '%s' at line %d ('%s')",
                        desiredOption, mountDirectory ? mountDirectory : "-", mountType ? mountType : "-", mountFileName, lineNumber, CheckMountOptions(mountStruct->mnt_opts));

                    if (NULL != mountDirectory)
                    {
                        OsConfigCaptureSuccessReason(reason, "Option '%s' for mount directory '%s' found in '%s' at line %d ('%s')",
                            desiredOption, mountDirectory, mountFileName, lineNumber, CheckMountOptions(mountStruct->mnt_opts));
                    }

                    if (NULL != mountType)
                    {
                        OsConfigCaptureSuccessReason(reason, "Option '%s' for mount type '%s' found in '%s' at line %d ('%s')",
                            desiredOption, mountType, mountFileName, lineNumber, CheckMountOptions(mountStruct->mnt_opts));
                    }
                }
                else
                {
                ...
                OsConfigLogDebug(log, "CheckFileSystemMountingOption, line %d in '%s': mnt_fsname '%s', mnt_dir '%s', mnt_type '%s', mnt_opts '%s', mnt_freq %d, mnt_passno %d",
                    lineNumber, mountFileName, mountStruct->mnt_fsname, mountStruct->mnt_dir, mountStruct->mnt_type, CheckMountOptions(mountStruct->mnt_opts),
            ...
            while (NULL != (mountStruct = getmntent(sourceHandle)))
            {
                if (0 != (status = addmntent(targetHandle, mountStruct)))
                {
                    OsConfigLogInfo(log, "CopyMountTableFile ('%s' to '%s'): failed adding '%s %s %s %s %d %d', addmntent() failed with %d (errno: %d)", source, target,
                        mountStruct->mnt_fsname, mountStruct->mnt_dir, mountStruct->mnt_type, CheckMountOptions(mountStruct->mnt_opts), mountStruct->mnt_freq, mountStruct->mnt_passno, status, errno);
                    break;
                }
            }
        }
                    ...
                    ((NULL != mountType) && (NULL != mountStruct->mnt_type) && (NULL != strstr(mountStruct->mnt_type, mountType))))
                {
                    ...
                    if (NULL != hasmntopt(mountStruct, desiredOption))
                    {
                        OsConfigLogInfo(log, "SetFileSystemMountingOption: option '%s' for mount directory '%s' or mount type '%s' already set in '%s' at line %d ('%s')",
                            desiredOption, mountDirectory ? mountDirectory : "-", mountType ? mountType : "-", fsMountTable, lineNumber, CheckMountOptions(mountStruct->mnt_opts));
                                ...
                                ((NULL != mountType) && (NULL != mountStruct->mnt_type) && (NULL != strstr(mountStruct->mnt_type, mountType))))
                            {
                                ...
                                if (NULL != hasmntopt(mountStruct, desiredOption))
                                {
                                    OsConfigLogInfo(log, "SetFileSystemMountingOption: option '%s' for mount directory '%s' or mount type '%s' found set in '%s' at line %d ('%s')",
                                        desiredOption, mountDirectory ? mountDirectory : "-", mountType ? mountType : "-", mountTable, lineNumber, CheckMountOptions(mountStruct->mnt_opts));
