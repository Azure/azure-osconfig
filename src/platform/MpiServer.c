...
{
    OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "Received MpiClose request, session %p ('%s')", handle, (char*)handle);
...
static int CallMpiSet(MPI_HANDLE handle, const char* componentName, const char* objectName, MPI_JSON_STRING payload, const int payloadSize)
{
    ...
    if (IsDebugLoggingEnabled())
    {
        if (MPI_OK == status)
        {
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiSet(%s, %s) request, session %p ('%s')", componentName, objectName, handle, (char*)handle);
        }
        else
        {
...
static int CallMpiGet(MPI_HANDLE handle, const char* componentName, const char* objectName, MPI_JSON_STRING* payload, int* payloadSize)
{
    ...
    if (IsDebugLoggingEnabled())
    {
        if (MPI_OK == status)
        {
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiGet(%s, %s) request, session %p ('%s')", componentName, objectName, handle, (char*)handle);
        }
        else
        {
...
static int CallMpiSetDesired(MPI_HANDLE handle, const MPI_JSON_STRING payload, const int payloadSize)
{
    ...
    if (IsDebugLoggingEnabled())
    {
        if (MPI_OK == status)
        {
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiSetDesired request, session %p ('%s')", handle, (char*)handle);
        }
        else
        {
...
static int CallMpiGetReported(MPI_HANDLE handle, MPI_JSON_STRING* payload, int* payloadSize)
{
    ...
    if (IsDebugLoggingEnabled())
    {
        if (MPI_OK == status)
        {
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiGetReported request, session %p ('%s')", handle, (char*)handle);
        }
        else
        {
            ...
            (0 == strcmp(uri, MPI_GET_REPORTED_URI)))
        {
            ...
            else if ((0 == strcmp(uri, MPI_SET_URI)) || (0 == strcmp(uri, MPI_GET_URI)))
            {
                ...
                else
                {
                    ...
                    {
                        status = SetErrorResponse(uri, mpiStatus, response, responseSize);
                        OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "%s(%s, %s): failed for client '%s' with %d (returning %d)", uri, component, object, client, mpiStatus, status);
                    }
                }
            }
...
static void* MpiServerWorker(void* arguments)
{
    ...
    while (g_serverActive)
    {
        ...
        if (0 <= (socketHandle = accept(g_socketfd, (struct sockaddr*)&g_socketaddr, &g_socketlen)))
        {
            ...
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "Accepted connection: path %s, handle '%d'", g_socketaddr.sun_path, socketHandle);
            ...
            if (HTTP_OK == status)
            {
                OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "%s: content-length %d, body, '%s'", uri, contentLength, requestBody);
            ...
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "Closed connection: path %s, handle '%d'", g_socketaddr.sun_path, socketHandle);
