...
static int ParseModuleInfo(const JSON_Value* value, MODULE_INFO** moduleInfo)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                OsConfigLogError(GetPlatformLog(), "[ParseModuleInfo] Failed to allocate memory for module license URI");
                status = errno;
            }

            if ((NULL != info->projectUri) && (NULL == (info->projectUri = strdup(info->projectUri))))
            {
                OsConfigLogError(GetPlatformLog(), "[ParseModuleInfo] Failed to allocate memory for module project URI");
                status = errno;
            }

            if (NULL == (info->components = (char**)malloc(componentsCount * sizeof(char*))))
            {
                OsConfigLogError(GetPlatformLog(), "[ParseModuleInfo] Failed to allocate memory for module components");
                status = ENOMEM;
            }
            else
            {
...
MODULE* LoadModule(const char* client, const char* path)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                OsConfigLogError(GetPlatformLog(), "[LoadModule] Function '%s()' not implmenented by '%s'", g_mmiGetInfoFunction, module->name);
                status = ENOENT;
            }

            if (NULL == (module->open = (MMI_OPEN)dlsym(module->handle, g_mmiOpenFunction)))
            {
                OsConfigLogError(GetPlatformLog(), "[LoadModule] Function '%s()' not implmenented by '%s'", g_mmiOpenFunction, module->name);
                status = ENOENT;
            }

            if (NULL == (module->close = (MMI_CLOSE)dlsym(module->handle, g_mmiCloseFunction)))
            {
                OsConfigLogError(GetPlatformLog(), "[LoadModule] Function '%s()' not implmenented by '%s'", g_mmiCloseFunction, module->name);
                status = ENOENT;
            }

            if (NULL == (module->get = (MMI_GET)dlsym(module->handle, g_mmiGetFunction)))
            {
                OsConfigLogError(GetPlatformLog(), "[LoadModule] Function '%s()' not implmenented by '%s'", g_mmiGetFunction, module->name);
                status = ENOENT;
            }

            if (NULL == (module->set = (MMI_SET)dlsym(module->handle, g_mmiSetFunction)))
            {
                OsConfigLogError(GetPlatformLog(), "[LoadModule] Function '%s()' not implmenented by '%s'", g_mmiSetFunction, module->name);
                status = ENOENT;
            }

            if (NULL == (module->free = (MMI_FREE)dlsym(module->handle, g_mmiFreeFunction)))
            {
                OsConfigLogError(GetPlatformLog(), "[LoadModule] Function '%s()' not implmenented by '%s'", g_mmiFreeFunction, module->name);
                status = ENOENT;
            }
