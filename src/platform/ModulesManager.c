            ...
            if (0 < reportedCount)
            {
                ...
                if (NULL != (reported = (REPORTED_OBJECT*)malloc(reportedSize)))
                {
                    ...
                    for (i = 0; i < reportedCount; i++)
                    {
                        ...
                        {
                            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "LoadModules: found reported property (%s.%s)", reported[i].component, reported[i].object);
                            reportedTotal++;
                        }
                    }
...
MPI_HANDLE MpiOpen(const char* clientName, const unsigned int maxPayloadSizeBytes)
{
    ...
    {
        OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiOpen: creating session with UUID '%s'", uuid);
...
void MpiClose(MPI_HANDLE handle)
{
    ...
    {
        OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiClose: closing session with UUID '%s'", session->uuid);
...
int MpiGet(MPI_HANDLE handle, const char* component, const char* object, MPI_JSON_STRING* payload, int* payloadSizeBytes)
{
    ...
    else
    {
        ...
        if (IsDebugLoggingEnabled())
        {
            if (MMI_OK == status)
            {
                OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiGet(%p, %s, %s, %p, %p) succeeded", handle, component, object, payload, payloadSizeBytes);
            }
            else
            {
...
int MpiSetDesired(MPI_HANDLE handle, const MPI_JSON_STRING payload, const int payloadSizeBytes)
{
    ...
    if (IsDebugLoggingEnabled())
    {
        if (MMI_OK == status)
        {
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiSetDesired(%p, %p, %d) succeeded", handle, payload, payloadSizeBytes);
        }
        else
        {
...
int MpiGetReported(MPI_HANDLE handle, MPI_JSON_STRING* payload, int* payloadSizeBytes)
{
    ...
    else
    {
        for (i = 0; i < g_reportedTotal; i++)
        {
            ...
            else
            {
                ...
                OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MmiGet(%s, %s) returned %d (%.*s)", g_reported[i].component, g_reported[i].object, mmiStatus, mmiPayloadSizeBytes, mmiPayload);
    ...
    if (IsDebugLoggingEnabled())
    {
        if (MMI_OK == status)
        {
            OsConfigLog(GetPlatformLog(), LoggingLevelDebug, "MpiGetDesired(%p, %p) succeeded", handle, payload);
        }
        else
        {
