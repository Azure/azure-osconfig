...
const std::string FirewallBase::m_firewallCreateRule = "createRule";
...
Rule Rule::FromJson(const std::string ruleJson)
{
    rapidjson::Document document;

    if (!document.Parse(ruleJson.c_str()).HasParseError())
    {

    }
    else
    {
        // TODO: log error and return empty rule
    }
}
...
int Firewall::Set(const char* componentName, const char* objectName, const MMI_JSON_STRING payload, const int payloadSizeBytes)
{
    ...
    {
        if (0 == m_firewallCreateRule.compare(objectName))
        {
            Rule rule = Rule::FromJson(std::string(payload, payloadSizeBytes));
            status = m_utility.Create(rule);
        }
        else
        {
