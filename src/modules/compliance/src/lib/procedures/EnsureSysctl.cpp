...
AUDIT_FN(EnsureSysctl, "sysctlName:Name of the sysctl:M:^([a-zA-Z0-9_]+[\\.a-zA-Z0-9_-]+)$", "value:Regex that the value of sysctl has to match:M")
{
    ...
    auto ufwDefaults = context.GetFileContents("/etc/defaults/ufw");
    if (!ufwDefaults.HasValue())
    {
        return indicators.NonCompliant("Failed to read /etc/defaults/ufw: " + ufwDefaults.Error().message);
    }
    ...
    if (sysctlFile.empty())
    {
        return indicators.NonCompliant("Failed to find IPT_SYSCTL in /etc/defaults/ufw");
    }
