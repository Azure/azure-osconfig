...
static bool PushDir(ListDirState& state, const std::string& relPath)
{
...
static int ListDirectoryIterator(lua_State* L)
{
...
static int LuaGetFilesystemEntriesWithPerms(lua_State* L)
{
    ...
    if (!view->ctx)
    {
        luaL_error(L, "internal error: null context in call context view");
        return 0;
    }
