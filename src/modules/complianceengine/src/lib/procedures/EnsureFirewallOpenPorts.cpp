...
AUDIT_FN(EnsureUfwOpenPorts)
{
    ...
    if (!foundSeparator)
    {
        return Error("Failed to start of rules in ufw status output");
    }
    ...
    while (std::getline(ufwStream, line))
    {
        ...
        // dest port/tcp ...
        // dest port
        // And there can be '(v6)' somewhere in the line....
    ...
    for (const auto& port : openPorts)
    {
        if (!port.IsLocal() && (port.family == AF_INET) && v4ports.find(port.port) == v4ports.end())
        {
            return indicators.NonCompliant("Port " + std::to_string(port.port) + " is open but not listed in ufw for ipv4");
        }
        if (!port.IsLocal() && (port.family == AF_INET6) && v6ports.find(port.port) == v6ports.end())
        {
