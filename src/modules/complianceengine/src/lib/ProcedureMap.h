...
struct Bindings<EnsureSshdOptionParams>
{
    ...
    static const char* names[];
    static constexpr auto members = std::make_tuple(&T::option, &T::value);
};

// Defines the bindings for the EnsureSshdNoOptionParams structure.
template <>
struct Bindings<EnsureSshdNoOptionParams>
{
    using T = EnsureSshdNoOptionParams;
    static constexpr size_t size = 2;
    static const char* names[];
    static constexpr auto members = std::make_tuple(&T::options, &T::values);
};
