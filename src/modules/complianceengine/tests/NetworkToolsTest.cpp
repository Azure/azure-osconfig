...
TEST_F(NetworkToolsTest, GetOpenPorts_WildcardAddress_ConvertsToZeros)
{
    std::vector<std::string> lines = {
        "tcp   LISTEN  0       128                *:80         0.0.0.0:*      users:((\"httpd\",pid=1234,fd=3))",
...
{
    std::vector<std::string> lines = {
        "tcp   LISTEN  0       128     192.168.1.100%eth0:22      0.0.0.0:*      users:((\"sshd\",pid=1234,fd=3))",
...
{
    std::vector<std::string> lines = {
        "tcp   LISTEN  0       128     192.168.1.1%br-docker0:80     0.0.0.0:*      users:((\"httpd\",pid=1234,fd=3))",
    ...
    EXPECT_EQ(ports[0].iface, "br-docker0");
    
    VerifyOpenPort(ports[1], AF_INET, SOCK_DGRAM, "10.0.0.1", 53);
    EXPECT_EQ(ports[1].iface, "veth12345ab");
    
    VerifyOpenPort(ports[2], AF_INET, SOCK_STREAM, "172.17.0.1", 8080);
    EXPECT_EQ(ports[2].iface, "docker_gwbridge");
}
...
TEST_F(NetworkToolsTest, GetOpenPorts_NoInterfaceSpecified_InterfaceEmpty)
{
    ...
    VerifyOpenPort(ports[1], AF_INET, SOCK_STREAM, "127.0.0.1", 3306);
    EXPECT_TRUE(ports[1].iface.empty());
