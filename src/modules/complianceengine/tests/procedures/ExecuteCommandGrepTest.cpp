...
TEST_F(ExecuteCommandGrepTest, AuditWithAwkTransformation)
{
    EXPECT_CALL(mContext,
        ExecuteCommand(::testing::HasSubstr("iptables -L -n | awk \"{print \\$1}\"  | grep -P -- \"test\" || (echo -n 'No match found'; exit 1)")))
...
TEST_F(ExecuteCommandGrepTest, AuditWithAwkAndExtendedRegex)
{
    EXPECT_CALL(mContext, ExecuteCommand(::testing::HasSubstr(
                              "iptables -L -n | awk \"{print \\$2}\"  | grep -E -- \"test.*pattern\" || (echo -n 'No match found'; exit 1)")))
...
TEST_F(ExecuteCommandGrepTest, AuditWithAwkSpecialCharactersEscaping)
{
    ...
    EXPECT_CALL(mContext, ExecuteCommand(::testing::HasSubstr(
                              "iptables -L -n | awk \"/^Chain/ {print \\$2}\"  | grep -P -- \"INPUT\" || (echo -n 'No match found'; exit 1)")))
...
{
    EXPECT_CALL(mContext, ExecuteCommand(::testing::HasSubstr("uname | awk \"BEGIN{FS=\\\"\\\\n\\\"} {gsub(/\\\\s+/, \\\"\\\", \\$1); print \\$1}\"  | "
...
TEST_F(ExecuteCommandGrepTest, AuditWithAwkFailsAtGrep)
{
    ...
    EXPECT_CALL(mContext, ExecuteCommand(::testing::HasSubstr(
                              "iptables -L -n | awk \"{print \\$3}\"  | grep -P -- \"nonexistent\" || (echo -n 'No match found'; exit 1)")))
