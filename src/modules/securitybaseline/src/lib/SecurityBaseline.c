...
static OsConfigLogHandle SecurityBaselineGetLog(void)
{
    return g_log;
}
...
int SecurityBaselineMmiGetInfo(const char* clientName, MMI_JSON_STRING* payload, int* payloadSizeBytes)
{
    ...
    *payload = (MMI_JSON_STRING)malloc(*payloadSizeBytes);
    ...
    OsConfigLog(SecurityBaselineGetLog(), LoggingLevelDebug, "MmiGetInfo(%s, %.*s, %d) returning %d", clientName, *payloadSizeBytes, *payload, *payloadSizeBytes, status);
