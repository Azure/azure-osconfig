        ...
        if (0 == CommandRunner::m_componentName.compare(componentName))
        {
            if (0 == g_commandArguments.compare(objectName))
            {
                ...
                if (m_usePersistedCache)
                {
                    ...
                    {
                        OsConfigLog(CommandRunnerLog::Get(), LoggingLevelDebug, "Updating command (%s) loaded from disk, with complete payload", arguments.m_id.c_str());
...
int CommandRunner::ScheduleCommand(std::shared_ptr<Command> command)
{
    ...
    {
        OsConfigLog(CommandRunnerLog::Get(), LoggingLevelDebug, "Command already recieved: %s (%s)", command->GetId().c_str(), command->m_arguments.c_str());
    }
...
void CommandRunner::WorkerThread(CommandRunner& instance)
{
    ...
    while (nullptr != (command = instance.m_commandQueue.Front().lock()))
    {
        ...
        {
            OsConfigLog(CommandRunnerLog::Get(), LoggingLevelDebug, "Command '%s' (%s) completed with code: %d", command->GetId().c_str(), command->m_arguments.c_str(), exitCode);
        }
        else
        {
...
int CommandRunner::LoadPersistedCommandStatus(const std::string& clientName)
{
    ...
    if (file.good())
    {
        ...
        {
            OsConfigLog(CommandRunnerLog::Get(), LoggingLevelDebug, "Cache file does not contain a status for client: %s", clientName.c_str());
        }
    }
