...
{
    return HostNameMmiGetInfo(clientName, payload, payloadSizeBytes);
}

MMI_HANDLE MmiOpen(const char* clientName, const unsigned int maxPayloadSizeBytes)
{
    return HostNameMmiOpen(clientName, maxPayloadSizeBytes);
}

void MmiClose(MMI_HANDLE clientSession)
{
    return HostNameMmiClose(clientSession);
}
...
{
    return HostNameMmiSet(clientSession, componentName, objectName, payload, payloadSizeBytes);
}
...
{
    return HostNameMmiGet(clientSession, componentName, objectName, payload, payloadSizeBytes);
}

void MmiFree(MMI_JSON_STRING payload)
{
    return HostNameMmiFree(payload);
}