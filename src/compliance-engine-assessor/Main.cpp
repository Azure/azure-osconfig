...
using ComplianceEngine::MmiFormatter;
...
int main(int argc, char* argv[])
{
    ...
    auto error = benchmarkFormatter->Begin(options.command == Command::Audit ? ComplianceEngine::Evaluator::Action::Audit :
                                                                               ComplianceEngine::Evaluator::Action::Remediate);
    if (error)
    {
    ...
    while (std::getline(inputStream, line))
    {
        ...
        {
            if (mofEntry.payloadKey.section.find(options.section.Value()) != 0)
            {
