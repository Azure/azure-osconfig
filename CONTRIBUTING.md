...
Pull requests needs to be formatted according to .pre-commit-config.yaml. Each pull request is checked by [Formatting Tests](https://github.com/Azure/azure-osconfig/blob/main/.github/workflows/formatting.yml).
...
4. When the source changes are staged, manually run [pre-commit](https://pre-commit.com/) with the following command:
```bash
